...
import numpy as np
import adet.utils.ann as pycococreatortools

ROOT_DIR = '/data8T/yl/Dataset/cityscapes'
save = '/data8T/yl/Dataset/cityscape'
IMAGE_DIR = os.path.join(ROOT_DIR, "leftImg8bit/val")
ANNOTATION_DIR = os.path.join(ROOT_DIR, "gtFine/val")
ANNOTATION_SAVE_DIR = os.path.join(save, "annotations")  # annotations是新创建的
INSTANCE_DIR = os.path.join(ROOT_DIR, "gtFine/val")
IMAGE_SAVE_DIR = os.path.join(save, "val_images")  # train_images是新创建的
...
def masks_generator(imges, ann_path):
    ...
    for pic_name in imges:
        ...
        ann_folder = os.path.join(INSTANCE_DIR, image_name).replace('cityscapes', 'cityscape')
        # os.mkdir(ann_folder)
...
def json_generate():
    ...
    for image_filename in files:
        ...
        ann_files = os.listdir(annotation_sub_path.replace('cityscapes', 'cityscape'))
        if len(ann_files) == 0:
            print("ao avaliable annotation")
            continue
        else:
            for annotation_filename in ann_files:
                annotation_path = os.path.join(annotation_sub_path.replace('cityscapes', 'cityscape'), annotation_filename)
                for x in CATEGORIES:
