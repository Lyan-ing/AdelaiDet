...
class Trainer(DefaultTrainer):
    ...
    def build_hooks(self):
        ...
        if comm.is_main_process():
            if self.cfg.MODEL.INTERRUPT:  # whether choose the checkpoint with interrupt
...
# from torchvision import models
    ...
    def compute_iou_with_each_box(self, true_boxes, pred_boxes, pred_scores):
    ...
    def merge_pseudo_label(self, ori_data, teacher_prediction):
        # 取出ture bbox和 pred bbox
        for single_data, single_pred in zip(ori_data, teacher_prediction):
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.MODEL.BOXINST.SEMSUPER[0]:  # 在此处,  data aug for different models(teacher and student)
            ...
            # if self.iter % self.cfg.TEST.EVAL_PERIOD == 0:  # 1
            ...
            with torch.no_grad():
                ...
                teacher_prediction= self.model_teacher(data, branch="teacher")

            #  Pseudo-labeling
            # cur_threshold = self.threshold

            data = self.merge_pseudo_label(data, teacher_prediction)
