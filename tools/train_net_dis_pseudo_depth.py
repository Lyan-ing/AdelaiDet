...
class Trainer(DefaultTrainer):
    ...
    def compute_iou_with_each_box(self, true_boxes, pred_boxes, pred_scores, mask_score_with_depth_sim=None, semi_rate=None):
        num_pred_box = true_boxes.shape[0]
        # match_idx = []
        # match_score =[]
        pred_boxes = pred_boxes.cpu()
        ggiou = generalized_box_iou(true_boxes, pred_boxes)
        # ggiou = (self.threshold[1] + semi_rate) * ggiou + (
        #             1 - (self.threshold[1] + semi_rate)) * mask_score_with_depth_sim
    ...
    def merge_pseudo_label(self, ori_data, teacher_prediction, mask_feature_attention):
        ...
        for single_data, single_pred, mask_feature in zip(ori_data, teacher_prediction,
                                                                         mask_feature_attention):
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.MODEL.BOXINST.SEMSUPER[0]:  # 在此处,  data aug for different models(teacher and student)
            ...
            # semi_rate = (self.iter / self.cfg.SOLVER.MAX_ITER) * 0.5

            with torch.no_grad():
                # (teacher_prediction, depth_sims) = self.model_teacher(data, branch="teacher")
                (teacher_prediction, mask_feature_attention, _) = self.model_teacher(data, branch="teacher")


            data = self.merge_pseudo_label(data, teacher_prediction, mask_feature_attention)
