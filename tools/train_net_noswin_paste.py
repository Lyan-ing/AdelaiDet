...
from adet.checkpoint import Checkpoint_With_Interrupt
...
#         if cfg.MODEL.BOXINST.SEMSUPER[0]>cfg.SOLVER.MAX_ITER:
    ...
    def resume_or_load(self, resume=True):
        ...
        if resume and self.checkpointer.has_checkpoint():
            ...
            # self._trainer.model.mask_head._iter = torch.tensor([self.start_iter],
            #                                                    device=self._trainer.model.mask_head._iter.device)
    ...
    def train_loop(self, start_iter: int, max_iter: int):
        ...
        self.model.mask_head._iter = torch.tensor([self.start_iter],
                                                               device=self.model.mask_head._iter.device)
    ...
    def merge_pseudo_label(self, ori_data, teacher_prediction, depth_sims, semi_rate):
        ...
        for single_data, single_pred, depth_sim in zip(ori_data, teacher_prediction, depth_sims):
            ...
            single_data["instances"].pseudo_masks = torch.zeros((ins_num, img_size0, img_size1))
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.MODEL.BOXINST.SEMSUPER[0]:  # 在此处,  data aug for different models(teacher and student)
            ...
            data = self.merge_pseudo_label(data, teacher_prediction, depth_sims, semi_rate)
            # print(data[0]['file_name'])
            # data = self.merge_pseudo_label(data, teacher_prediction, mask_feature_attention, depth_sims)
            paste_choices = ['min', 'max', 'depth']
            paste_choice = [paste_choices[i] for i in self.cfg.MODEL.BOXINST.PASTE_CHOICE]
            if random.random() < 0.5:
                new_merged_data = copy_and_paste(data, data[::-1], paste_choice)
            else:
                new_merged_data = copy_and_paste(data[::-1], data, paste_choice)
            if len(new_merged_data) > 0:
                data.extend(new_merged_data)
