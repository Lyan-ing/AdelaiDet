    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None):
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                # depth_prediction = torch.stack(gt_inds.depth, dim=0)
                # if self.bg_depth:
                ...
                if self.boxinst_enabled:
                    ...
                    if self.depth_thresh:
                        # im_inds = pred_instances.im_inds  # 实例对应图像索引
                        # gt_depth = depth_prediction[im_inds].to(dtype=mask_feats.dtype)  # 每张图对应的gt_depth
                        gt_depth = torch.cat([x.depth for x in gt_instances])
                        gt_depth = gt_depth[gt_inds].to(dtype=mask_feats.dtype)
                        gt_depth = gt_depth.unsqueeze(1)
                        depth_edge = torch.cat([x.depth_edge for x in gt_instances])
                        depth_edge = depth_edge[gt_inds].to(dtype=mask_feats.dtype)
                        # ts.save(depth_edge[0], file_root + '/depth_edge.png')
                        depth_bitmask = deepcopy(gt_bitmasks)
                        depth_edge_matrix = depth_edge * depth_bitmask.float()
                        edge_sum = depth_edge_matrix.sum(dim=(1, 2, 3))
                        depth_bitmask[edge_sum < 5] = 0  # 删去小边界
                        # ts.save(gt_bitmasks[0], file_root+'/gt_bitmask.png')
                        depth_edge_value = depth_edge_matrix * gt_depth
                        depth_edge_value_no_zero = deepcopy(depth_edge_value)
                        depth_edge_value_no_zero[depth_edge_value < 5] = 255
                        depth_edge_min_h = (depth_edge_value_no_zero.min(2, True))[0].expand(
                            depth_edge_value.shape)
                        depth_edge_min_w = (depth_edge_value_no_zero.min(3, True))[0].expand(
                            depth_edge_value.shape)
                        depth_edge_min = (torch.cat((depth_edge_min_h, depth_edge_min_w), dim=1).max(1, True))[0]
                        depth_bitmask[gt_depth <= (depth_edge_min)] = 0
                        # ts.save(depth_edge_value[0], file_root + '/depth_edge_value.png')
                        # ts.save(depth_bitmask[0], file_root+'/depth_bitmask_min.png')

                        # depth_edge_mean_h = (
                        #         depth_edge_value.sum(2, True) / depth_edge_matrix.sum(2, True).clamp(min=1)).expand(
                        #     depth_edge_value.shape)
                        # depth_bitmask[gt_depth <= depth_edge_mean_h] = 0
                        #
                        # depth_edge_mean_w = (
                        #         depth_edge_value.sum(3, True) / depth_edge_matrix.sum(3, True).clamp(min=1)).expand(
                        #     depth_edge_value.shape)
                        # depth_bitmask[gt_depth <= depth_edge_mean_w] = 0
                        # 过滤过高的异常区域，通过中轴线确定实例内部基础值
                        # 1. 通过边线均值确定
                        depth_edge_mean_h = (
                                depth_edge_value.sum(2, True) / depth_edge_matrix.sum(2, True).clamp(min=1)).expand(
                            depth_edge_value.shape)
                        depth_bitmask[gt_depth >= (depth_edge_mean_h + 15)] = 0

                        depth_edge_mean_w = (
                                depth_edge_value.sum(3, True) / depth_edge_matrix.sum(3, True).clamp(min=1)).expand(
                            depth_edge_value.shape)
                        depth_bitmask[gt_depth >= (depth_edge_mean_w + 15)] = 0
                        # ts.save(depth_bitmask[0], file_root + '/depth_bitmask_mean.png')
                        # loss_edge = (pairwise_losses * depth_bitmask).mean(1).sum() / depth_bitmask.sum().clamp(min=1)
                        # loss_edge = torch.log(loss_edge+1)

                        # 过滤过高的异常区域，通过中轴线确定实例内部基础值
                        # # 2. 计算中轴线：box/2
                        # gt_box = torch.cat([x.gt_boxes.tensor for x in gt_instances])
                        # gt_box = gt_box[gt_inds].to(dtype=mask_feats.dtype)
                        # box_w = ((gt_box[:, 0] + gt_box[:, 2]) / 8).int()
                        # box_h = ((gt_box[:, 1] + gt_box[:, 3]) / 8).int()
                        #
                        # box_h_depth = (
                        #     torch.cat([gt_depth[i, :, box_h[i]] for i in range(len(box_h))], dim=0)).unsqueeze(1)
                        # box_w_depth = (
                        #     torch.cat([gt_depth[i, :, :, box_w[i]] for i in range(len(box_w))], dim=0)).unsqueeze(2)
                        # box_h_depth = box_h_depth.unsqueeze(1)
                        # box_w_depth = box_w_depth.unsqueeze(1)
                        # box_axis_depth = \
                        #     torch.cat(
                        #         (box_w_depth.expand(depth_bitmask.shape), box_h_depth.expand(depth_bitmask.shape)),
                        #         dim=1).max(1, True)[0]
                        # depth_bitmask[gt_depth > (box_axis_depth + 25)] = 0

                        # ts.save(depth_bitmask[0], file_root + '/depth_bitmask_minmax.png')

                        # 过滤未找到边界的部分
                        # if
                        filter_no_edge_bitmask = deepcopy(depth_bitmask)
                        ori_bitmask_sum_h = ((gt_bitmasks.sum(2, True))[0]).expand(gt_bitmasks.shape)
                        end_bitmask_sum_h = ((depth_bitmask.sum(2, True))[0]).expand(gt_bitmasks.shape)
                        filter_no_edge_bitmask[end_bitmask_sum_h > (ori_bitmask_sum_h - 1)] = 0

                        ori_bitmask_sum_w = ((gt_bitmasks.sum(3, True))[0]).expand(gt_bitmasks.shape)
                        end_bitmask_sum_w = ((depth_bitmask.sum(3, True))[0]).expand(gt_bitmasks.shape)
                        filter_no_edge_bitmask[end_bitmask_sum_w > (ori_bitmask_sum_w - 1)] = 0

                        loss_edge = F.l1_loss(mask_scores * filter_no_edge_bitmask, filter_no_edge_bitmask, reduction='sum')
                        loss_edge = loss_edge / filter_no_edge_bitmask.sum().clamp(min=1)
                        # ts.save(filter_no_edge_bitmask[0], file_root + '/filter_no_edge_bitmask.png')
                        ...
                        losses.update({
                            'loss_edge': loss_edge * self.hpyer_parameters[1] * warmup_factor,  # edge
