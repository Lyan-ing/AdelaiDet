        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    pairwise = (pairwise_losses * weights).sum(dim=(1,2,3)) / weights.sum(dim = (1,2,3)).clamp(min=1.0)
                    ...
                    if self.pairwise_depth_thresh:
                        ...
                        depth_pairwise = (pairwise_losses * depth_weights).sum(dim=(1, 2, 3)) / depth_weights.sum(
                            dim=(1, 2, 3)).clamp(min=1.0)
                        loss_depth_pairwise = torch.stack(
                            [(depth_pairwise[gt_inds == i]).mean() for i in gt_inds.unique()]).mean()
                        # loss_depth_pairwise = ((pairwise_losses * image_depth_similarity)[
                        #     depth_weights.bool()]).sum() / depth_weights.sum().clamp(
                        #     min=1.0)
                        # warmup_factor2 = min((self._iter.item() - 10000) / float(self._warmup_iters), 1.0)
                        loss_depth_pairwise = loss_depth_pairwise * self.hpyer_parameters[3]
                        losses.update({
                            'loss_depth_pairwise': loss_depth_pairwise,
                        })

                    # loos_bg = (mask_scores*(1-mask_scores.detach()))[gt_edge]
                    # mask_bg = mask_scores[gt_edge]
                    # loss_bg =  mask_bg.mean()
