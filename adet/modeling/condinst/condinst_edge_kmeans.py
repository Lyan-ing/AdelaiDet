...
from .dynamic_mask_head_kmeans import build_dynamic_mask_head
from .mask_branch import build_mask_branch
import torchshow as ts
from adet.utils.comm import aligned_bilinear
from torchvision.utils import draw_bounding_boxes

__all__ = ["MyCondInst_Kmeans_Edge_Depth"]
...
class MyCondInst_Kmeans_Edge_Depth(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.use_depth and self.training:
            ...
            # file_root = './we_test/17/' + file_name.split('.')[0]
            ...
            if depth_prediction_down is not None:
                ...
                if self._iter >= self.kmean_v[0]:
                    ...
                    for i in range(len(per_im_gt_inst)):
                        ...
                        box_mean1 = deepcopy(box)
                        if box.sum() > 400:
                            img_2D1 = depth_prediction_down[im_i, 0][box == 1]
                            cluster_ids_x1, cluster_centers1, flag1 = kmeans(img_2D1.unsqueeze(dim =1), num_clusters=2,
                                                                             max_itrs=self.kmean_v[1],
                                                                             device=self.device)
                            box_mean1[box == 1] = (cluster_ids_x1 - 0.5) * 2
                            if flag1:
                                box_mean1[:] = 0
                            #     # print('true')
                            #     # print(str(file_root))
                            # ts.save(box_mean1 + 1, file_root + f'/kmeans_dir_{i}.png')
                            # # ts.save(box_mean + 1, file_root + f'/kmeans_{i}.png')
                            ...
                            # cluster_ids_x, cluster_centers, flag = kmeans(img_2D, num_clusters=2,
                            #                                               max_itrs=self.kmean_v[1],
                            #                                               device=self.device)
                            # # box_mean = deepcopy(box)
                        ...
                        if kmeans_box is None:
                            kmeans_box = box_mean1[None]
                        else:
                            kmeans_box = torch.cat((kmeans_box, box_mean1[None]), dim=0)
