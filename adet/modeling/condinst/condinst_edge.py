...
from .dynamic_mask_head_edge import build_dynamic_mask_head
from .mask_branch import build_mask_branch
import torchshow as ts
from adet.utils.comm import aligned_bilinear
from torchvision.utils import draw_bounding_boxes

__all__ = ["MyCondInst_Edge_Depth"]
...
def get_images_color_similarity(images, image_masks, kernel_size, dilation, sim=0, edge=0):
    assert images.dim() == 4
    ...
    if images.size(1) == 1:
        if sim and edge:
            # edge = diff.unsqueeze(1) * unfolded_weights
            return similarity * unfolded_weights, torch.max(diff.squeeze(1) * unfolded_weights, dim=1)[0]
        elif sim:
            return similarity * unfolded_weights, 0
        elif edge:
            return 0, torch.max(diff.squeeze(1) * unfolded_weights, dim=1)[0]
    else:
        return similarity * unfolded_weights


@META_ARCH_REGISTRY.register()
class MyCondInst_Edge_Depth(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances, depth_prediction)
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances, depth_prediction):
        ...
        loss_mask = self.mask_head(
            ...
            pred_instances, gt_instances, depth_prediction
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            if depth_prediction is not None:
                image_depth_similarity, depth_edge = get_images_color_similarity(
                    ...
                    self.pairwise_size, self.pairwise_dilation, self.pairwise_depth_thresh, self.depth_thresh
                )
