...
class DynamicMaskHead(nn.Module):
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None, depth_map=None,
        ...
        if self.training and branch == 'student':
            ...
            # if gt_instances[0].has('pred_mask'):
            #     pseudo_masks = torch.cat([per_im.pred_masks for per_im in gt_instances])
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    if gt_instances[0].has('pseudo_masks'):
                        ...
                        loss_pseudo_masks = (loss_pseudo_masks * pseudo_scores).mean() * 10

                        losses.update({
                            'loss_pseudo_masks': loss_pseudo_masks,
                        })
                    ...
                    pairwise = (pairwise_losses * weights).sum(dim=(1, 2, 3)) / weights.sum(dim=(1, 2, 3)).clamp(
                        min=1.0)
                    loss_pairwise = torch.stack([(pairwise[gt_inds == i]).mean() for i in gt_inds.unique()]).mean()
                    # warmup_factor = min(self._iter.item() / float(self._warmup_iters), 1.0)
                    ...
                    if (self._iter > self._warmup_iters) and self.pairwise_depth_thresh:
                        ...
                        #     min=1.0)
                        loss_depth_pairwise = (pairwise_losses * depth_weights).sum(dim=(1, 2, 3)) / depth_weights.sum(
                            dim=(1, 2, 3)).clamp(min=1.0)
                        loss_depth_pairwise = torch.stack(
                            [(loss_depth_pairwise[gt_inds == i]).mean() for i in gt_inds.unique()]).mean()
                    ...
                    if self.hpyer_parameters[2]:
                        mask_bg = mask_scores[gt_edge]
                        loss_bg = (mask_bg - 0.01).pow(2).mean()
                        # loss_bg = (mask_bg * (1-mask_bg.detach())).mean()
                        losses.update({
                            'loss_bg': loss_bg * self.hpyer_parameters[2],
                        })
