...
def parse_dynamic_params(params, channels, weight_nums, bias_nums):
    ...
    # (18,8)-->(8,8)-->(8,1) channel change  [18*8=144, 8*8=64,8*8=64, 8*1=8, 8*1=8] weight  [8, 8, 8, 1, 1] bias
    ...
    def mask_heads_forward(self, features, weights, biases, num_insts):
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):  # 1*1卷积
            ...
            if i==3:  # 18->8 depth
                ...
                depth = F.relu(depth)
            if i==4:  # 8->8 depth to mask,
                depth_mask = F.conv2d(
                    depth, w, bias=b,
                    stride=1, padding=0,
                    groups=num_insts
                )
                mask_merge = F.relu(depth_mask) + mask
            if i==5:  # 8->8 mask to depth
                mask_depth = F.conv2d(
                    mask, w, bias=b,
                    stride=1, padding=0,
                    groups=num_insts
                )
                depth_merge = F.relu(mask_depth) + depth
                # mask = F.conv2d(mask, w, bias=b,
                #     stride=1, padding=0,
                #     groups=num_insts)
                # rmp = F.relu(mask) + depth
            if i == 6:  # 8->1 depth
                depth_merge = F.conv2d(
                    depth_merge, w, bias=b,
                    stride=1, padding=0,
                    groups=num_insts
                )
                # depth = depth.sigmoid()
            ...
            if i == 7:  # 8->1 mask
                ...
                # x = x * depth
        return mask_merge, depth_merge.sigmoid()  # mask head output
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    if (self._iter > self._warmup_iters) and self.pairwise_depth_thresh:
                        ...
                        loss_depth_pairwise = (pairwise_losses * depth_weights * image_depth_similarity).sum() / depth_weights.sum().clamp(
                            min=1.0)
