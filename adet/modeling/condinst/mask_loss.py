...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.mask_parameters = cfg.MODEL.BOXINST.PAIRWISE.MASKS
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None, depth_map=None,
        ...
        if self.training and branch == 'student':
            ...
            box_gt_bitmasks = gt_bitmasks = gt_bitmasks[gt_inds].unsqueeze(dim=1).to(dtype=mask_feats.dtype)
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    if gt_instances[-1].has('pseudo_masks'):
                        ...
                        mask_flags = pseudo_scores = pseudo_scores[gt_inds].to(dtype=mask_feats.dtype)
                        # if self.tmp:
                        ...
                        losses.update({
                            'loss_pseudo_masks': mask_scores.sum() * 0.,
                        })
                        # losses.update({
                        #     'loss_pseudo_depths': depth_logits.mean() * 0.,
                        # })
                    ...
                    if (self._iter > self._warmup_iters) and self.pairwise_depth_thresh:
                        ...
                        if gt_instances[-1].has('pseudo_masks'):
                            ...
                            # else:
                            mask_affinity_weights = (mask_affinity >= self.mask_parameters[0]).float() * box_gt_bitmasks.float()

                            loss_mask_affinity = (pairwise_losses * mask_affinity_weights).sum(
                                [1, 2, 3]) / mask_affinity_weights.sum([1, 2, 3]).clamp(min=1.0)
                            # NOTE: weight???
                            loss_mask_affinity = (loss_mask_affinity[pseudo_scores > self.semi_threshold[0]]).mean()  # (loss_mask_affinity * pseudo_mask_weight).sum() / num_valid_masks

                            losses[
                                "loss_mask_affinity"] = loss_mask_affinity * self.mask_parameters[1]  # self.pseudo_mask_affinity_weight
