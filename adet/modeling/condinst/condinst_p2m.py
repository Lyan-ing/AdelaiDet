...
from .dynamic_mask_head_p2m import build_dynamic_mask_head
from .mask_branch_p2m import build_mask_branch
import torchshow as ts
from adet.utils.comm import aligned_bilinear
from torchvision.utils import draw_bounding_boxes

__all__ = ["MyCondInst_P2B"]
...
def grad(x):
    return torch.stack(torch.gradient(x), dim=0)


def norm(x, axis=0):
    return torch.sqrt(torch.sum(torch.square(x), dim=axis))
...
class MyCondInst_P2B(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        # self.nn_emb = nn.Embedding().detach()
    ...
    def forward(self, batched_inputs):
        ...
        if self.use_depth and self.training:
            # tmp_img = batched_inputs[0]["image"].unsqueeze(0)[:, [2, 1, 0]].squeeze(0).cpu()
            # tmp_box = batched_inputs[0]['instances'].gt_boxes
            # # result1 = draw_bounding_boxes(tmp_img.unsqueeze(0)[:,[2,1,0]].squeeze(0).cpu(), tmp_box[0].tensor, colors=["red"], width=1)
            # # for i in range(1, len(tmp_box)):
            # result1 = draw_bounding_boxes(tmp_img, tmp_box.tensor, width=3)
            # file_name = batched_inputs[0]['file_name'].split('/')[-1]
            # file_root = './we_test/15/' + file_name.split('.')[0]
            # if not os.path.exists(file_root):
            #     os.mkdir(file_root)
            # ts.save(result1, file_root + '/ori.png')
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.boxinst_enabled:
                ...
                if self.use_depth and self.training:
                    ...
                    depth_prediction = depth_maps_norm.tensor.float()
                    self.add_bitmasks_from_boxes(
                        ...
                        depth_prediction=depth_prediction, file_root=file_root
                    )
                else:
                    self.add_bitmasks_from_boxes(
                        gt_instances, original_images.tensor, original_image_masks.tensor,
        ...
        if self.training:
            mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances, file_root)
            ...
            if self.pred_depth:
                ...
                depth_predictions = F.avg_pool2d(
                    depth_prediction, kernel_size=self.mask_out_stride,
                    stride=self.mask_out_stride, padding=0
                ) / 255.0
                depth_pred_loss["loss_depth_pred"] = F.l1_loss(depth_pred.sigmoid(),
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances, file_root):
        ...
        if self.max_proposals != -1:
            ...
            for im_id in range(num_images):
                ...
                num_instances_per_gt = max(int(self.topk_proposals_per_im / len(unique_gt_inds)), 1)  # 均匀保留，不公平
                # 只保留部分预测实例，每张图保留的实例数量有限制
        ...
        loss_mask, self._iter = self.mask_head(
            mask_feats, self.mask_branch.out_stride,
            pred_instances, gt_instances, file_root
        ...
        for per_im_gt_inst in instances:
            ...
            if isinstance(per_im_gt_inst.get("gt_masks"), PolygonMasks):
                ...
                for per_polygons in polygons:
                    ...
                    bitmask = bitmask[start::self.mask_out_stride, start::self.mask_out_stride]  # 不规则的mask下采样
    ...
    def add_bitmasks_from_boxes(self, instances, images, image_mask, im_h, im_w, depth_prediction=None, file_root=None):
        ...
        image_masks = image_mask[:, start::stride, start::stride]
        if depth_prediction is not None:
            depth_prediction_down = F.avg_pool2d(
                depth_prediction, kernel_size=stride,
                stride=stride, padding=0
            ).unsqueeze(dim=1)
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            if depth_prediction_down is not None:
                image_depth_similarity = get_images_color_similarity(
                    depth_prediction_down[im_i].unsqueeze(0), image_masks[im_i],
                    self.pairwise_size, self.pairwise_dilation
                )
            #     level_set = (stopping_fun(depth_prediction[im_i][0])).unsqueeze(0) * image_mask[im_i]
            #     depth_edge_ori = F.avg_pool2d(level_set[None], kernel_size=stride, stride=stride, padding=0)
            #     depth_edge = deepcopy(depth_edge_ori)
            #     depth_edge[depth_edge_ori < 0.5] = 1
            #     depth_edge[depth_edge_ori >= 0.5] = 0
            ...
            if depth_prediction_down is not None:
                ...
                #     ], dim=0)
                #     per_im_gt_inst.depth = torch.cat([
                #         depth_prediction_down[im_i] for _ in range(len(per_im_gt_inst))
                #     ], dim=0)

                # if self._iter >= self.kmean_v[0]:
                #     kmeans_box = None
                #     for i in range(len(per_im_gt_inst)):
                #         box = per_im_gt_inst.gt_bitmasks[i]
                #         box_mean = deepcopy(box)
                #         if box.sum() > 400:
                #             y0 = int((per_im_boxes[i][0] - 2) / 4)
                #             x0 = int((per_im_boxes[i][1] - 2) / 4)
                #             y1 = int((per_im_boxes[i][2] - 2) / 4)
                #             x1 = int((per_im_boxes[i][3] - 2) / 4)
                #             len_x = x1 - x0 + 1
                #             len_y = y1 - y0 + 1
                #             crop_img = depth_prediction_down[im_i][0, x0:x1 + 1, y0:y1 + 1]
                #             img_2D = crop_img.reshape(-1, 1)
                #             cluster_ids_x, cluster_centers, flag = kmeans(img_2D, num_clusters=2, max_itrs=self.kmean_v[1],
                #                                                           device=self.device)
                #             # box_mean = deepcopy(box)
                #             box_mean[x0:x1 + 1, y0:y1 + 1] = (cluster_ids_x.reshape(len_x, len_y) - 0.5) * 2
                #             if flag:
                #                 box_mean[:] = 0
                #             box_mean = box_mean * box
                #         else:
                #             box_mean[:] = 0
                #         if kmeans_box is None:
                #             kmeans_box = box_mean[None]
                #         else:
                #             kmeans_box = torch.cat((kmeans_box, box_mean[None]), dim=0)
                #     per_im_gt_inst.depth_kmean = kmeans_box
                    # ts.save(kmeans_box + 1, file_root + '/all_kmeans.png')
