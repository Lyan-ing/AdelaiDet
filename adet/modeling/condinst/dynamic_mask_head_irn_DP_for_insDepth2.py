...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        for l in range(self.num_layers):
            if l == 0:
                ...
                bias_nums.append(1)
                bias_nums.append(1)
                weight_nums.append(2)  # change the kernel to double
                bias_nums.append(1)
            else:
    ...
    def mask_heads_forward(self, features, weights, biases, num_insts):
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):  # 1*1卷积
            ...
            if i == n_layers - 1:
                x = torch.cat([torch.stack((x[0, i], depth[0, i])) for i in range(x.shape[1])], dim=0).unsqueeze(0)
        ...
        return x, depth.sigmoid()  # mask head output
        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    # depth_pred
                    norm_depth = (gt_depth / 255.0) * gt_bitmasks
