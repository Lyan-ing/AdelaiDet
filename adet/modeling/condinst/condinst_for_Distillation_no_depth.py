...
from .dynamic_mask_head_nodepth import build_dynamic_mask_head
...
__all__ = ["MyCondInst_Dis_no_depth"]
...
class MyCondInst_Dis_no_depth(nn.Module):
    """
    ...
    def preprocess_image(self, batched_inputs):
        ...
        return images_norm, original_images

    def forward(self, batched_inputs, branch='student'):
        if self.training and branch == 'student':
            images_norm, original_images = self.preprocess_image(batched_inputs)
            ...
            for im_idx, images_single in enumerate(images_norm):
                ...
                min_size = 1024
                max_size = max(teacher_size) * 1024 / min(teacher_size)
                if max_size > 2048:
                    max_size = 2048
        ...
        mask_feats, sem_losses, _ = self.mask_branch(features, gt_instances)
