...
__all__ = ["MyCondInst_Dis_feature"]
...
class MyCondInst_Dis_feature(nn.Module):
    """
    ...
    def forward(self, batched_inputs, branch='student'):
        ...
        if self.training:
            # mask_feats_attention = mask_feats.clone()
            mask_feats_attention = [single[:, :size[0] // 8, :size[1]// 8] for (single, size) in
                                    zip(mask_feats, original_image_masks.image_sizes)]
...
#                 losses.update(depth_pred_loss)
            ...
            if 'mask_feature' in batched_inputs[-1]:
                ...
                for (teacher_feature, student_feature) in zip(teacher_mask_feature, mask_feats_attention):
                    student_feature = F.interpolate(student_feature[None], size=teacher_feature.size()[1:],
                    ...
                    loss_feature = (student_feature - teacher_feature).pow(2).sum(0).mean()
                    loss_features.append(loss_feature)
                loss_features = torch.stack(loss_features).sum()
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            if per_im_gt_inst.has('pseudo_masks'):
                ...
                mask_affinity = get_instance_masks_affinity(
                    per_pseudo_masks, image_masks[im_i],
                    self.pairwise_size, self.pairwise_dilation)
