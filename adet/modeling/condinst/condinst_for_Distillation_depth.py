...
from .dynamic_mask_head_for_dis import build_dynamic_mask_head
...
def get_images_color_similarity(images, image_masks, kernel_size, dilation, sim=0.1):
    ...
    if images.size(1) == 1:
        ...
        edge = depth_sim.max(dim=1)[0]
        edge = (edge < sim)
        return depth_sim, edge
...
class MyCondInst_Dis_depth(nn.Module):
    """
    ...
    def forward(self, batched_inputs, branch='student'):
        ...
        if self.use_depth and self.training and branch == 'student':
            ...
            for im_idx, (images_single, depth_single) in enumerate(zip(images_norm, original_depths)):
                ...
                min_size = 800
                max_size = max(teacher_size) * 800 / min(teacher_size)
                if max_size > 1312:
                    max_size = 1312
                ...
                if teacher_size[0] > teacher_size[1]:
                    ...
                    # depth = F.interpolate(depth_single[None, None].float(), size=(max_size//4, min_size//4),
                    #               mode="bilinear", align_corners=False)[0,0]
                    # images_norm[im_idx] = F.pad(images_norm_tensor,
                    #                            pad=[0, int(32 - min_size % 32 if min_size % 32 else 0),
                    #                                 0, int(32 - max_size % 32 if max_size % 32 else 0)]).squeeze(0)
                    # pad=[0, int(32 - max_size % 32 if max_size % 32 else 0),
                    #      0, int(32 - min_size % 32 if min_size % 32 else 0)])
                else:
                    images_norm[im_idx] = F.interpolate(images_single[None], size=(min_size, max_size),
                                                       mode="bilinear", align_corners=False).squeeze(0)
                    # depth = F.interpolate(depth_single[None, None].float(), size=(min_size//4, max_size//4),
                    #                                         mode="bilinear", align_corners=False)
                    # images_norm[im_idx] = F.pad(images_norm_tensor,
                    #                            pad=[0, int(32 - max_size % 32 if max_size % 32 else 0),
                    #                                 0, int(32 - min_size % 32 if min_size % 32 else 0)]).squeeze(0)
        ...
        if self.training and branch == 'student':
            ...
            # loss_mask_feature = {}
            # if 'mask_feature' in batched_inputs[-1]:
            #     teacher_mask_feature = torch.stack(teacher_mask_feature, dim=0)
            #     teacher_mask_feature = F.interpolate(teacher_mask_feature[None],
            #                                          size=(images_norm.tensor.shape[-2] //8,
            #                                          images_norm.tensor.shape[-1]//8),
            #                                          mode="bilinear", align_corners=False).squeeze(0)
            #     teacher_mask_feature = F.normalize(teacher_mask_feature.view(teacher_mask_feature.shape[0], -1)).view(teacher_mask_feature.shape)
            #     mask_feats_attention = F.normalize(mask_feats_attention.view(mask_feats_attention.shape[0], -1)).view(mask_feats_attention.shape)
            #     loss_mask_feature['loss_mask_feature'] = (mask_feats_attention - teacher_mask_feature).pow(2).mean(dim=(1, 2)).mean() * 100 * self.mask_feature
            # else:
            #     loss_mask_feature['loss_mask_feature'] = mask_feats_attention.sum() * 0.
            ...
            return processed_results, depth_sims
        else:
