...
def compute_pairwise(mask_scores, pairwise_size, pairwise_dilation):
    assert mask_scores.dim() == 4

    # log_fg_prob = F.logsigmoid(mask_logits)
    # log_bg_prob = F.logsigmoid(-mask_logits)

    pairwise_diff = unfold_wo_center(
        mask_scores, kernel_size=pairwise_size,
        dilation=pairwise_dilation
    ).squeeze(1) - mask_scores
    # log_bg_prob_unfold = unfold_wo_center(
    #     log_bg_prob, kernel_size=pairwise_size,
    #     dilation=pairwise_dilation
    # )
    pairwise_diff = 1-pairwise_diff.abs()
    # the probability of making the same prediction = p_i * p_j + (1 - p_i) * (1 - p_j)
    # we compute the the probability in log space to avoid numerical instability
    # log_same_fg_prob = log_fg_prob[:, :, None] + log_fg_prob_unfold
    # log_same_bg_prob = log_bg_prob[:, :, None] + log_bg_prob_unfold
...
def comput_irn_loss(mask_scores, gt_edge, gt_bitmasks, dilation):
    ...
    # gt_bitmasks = gt_bitmasks  gt_edge.tile(1,8,1,1).unsqueeze(1)
...
def comput_irn_loss_tmp(mask_scores, gt_edge, gt_bitmasks, dilation):
    ...
    mask_edge = edge_diff[edge_diff != 0]
    if len(mask_edge) == 0:
        loss_edge = mask_score.sum() * 0. + 0.1
        # print(mask_score.sum())
        # print("**************************")
    else:
        # print(mask_edge.mean() / 4)
        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    pairwise_diff = compute_pairwise(
                        mask_scores, self.pairwise_size,
                        self.pairwise_dilation
                    )
                    ...
                    if self.pairwise_depth_thresh:  # (self._iter > self._warmup_iters) and
                        ...
                        if self.depth_with_box:
                            depth_weights = (depth_weights * gt_bitmasks.float()).bool()
                        loss_depth_pairwise = ((pairwise_losses * image_depth_similarity)[depth_weights]).sum() / depth_weights.sum().clamp(
                            min=1.0)
                        ...
                        depth_weights2 = (image_depth_similarity <= 0.3).float()
                        if self.depth_with_box:
                            depth_weights2 = depth_weights2 * gt_bitmasks.float() * (1 - weights)
                            depth_weights2 = depth_weights2.bool()
                        loss_depth_pairwisebg = ((pairwise_diff * (1 - image_depth_similarity))[depth_weights2]).sum() / depth_weights2.sum().clamp(min=1.0)
                        # if loss_depth_pairwisebg< 1e-5:
                        #     print(loss_depth_pairwisebg)
                        # elif loss_depth_pairwisebg > (1-1e-5):
                        # print(loss_depth_pairwisebg) * (1- image_depth_similarity) .clamp(min = 1e-5, max=0.99)
                        # a = pairwise_losses.sum()
                        # loss_depth_pairwisebg = 1 - torch.log(loss_depth_pairwisebg + 1)
                        # warmup_factor2 = min((self._iter.item() - 10000) / float(self._warmup_iters), 1.0)
                        loss_depth_pairwisebg = loss_depth_pairwisebg * warmup_factor * 0.1
                        losses.update({
                            'loss_depth_pairwisebg': loss_depth_pairwisebg,
                        })
