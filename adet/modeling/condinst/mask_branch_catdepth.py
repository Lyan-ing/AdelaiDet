...
class MaskBranch(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        if self.pred_depth:
            ...
            self.depth_head = nn.Sequential(
                conv_block(in_channels, channels, kernel_size=3, stride=1),
                conv_block(channels, 1, kernel_size=3, stride=1)
            )
    ...
    def forward(self, features, gt_instances=None):
        ...
        depth_pred = None
        if self.pred_depth:
            depth_pred = self.depth_head(features[self.in_features[0]])
            depth_pred = depth_pred.sigmoid()
            x = torch.cat((x, depth_pred), dim=1)
