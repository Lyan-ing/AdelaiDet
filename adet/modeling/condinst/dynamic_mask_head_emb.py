...
def parse_dynamic_params(params, channels, weight_nums, bias_nums):
    ...
    for l in range(num_layers):
        if l < num_layers - 1:
            ...
            # out_channels x in_channels x 1 x 1
            weight_splits[l] = weight_splits[l].reshape(num_insts * 1, -1, 3, 3)  # change the kernel to 3*3
...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.classes_mask_emb = nn.Sequential(
            nn.Conv2d(1,3)
        )
        ...
        for l in range(self.num_layers):
            if l == 0:
                ...
                weight_nums.append(self.channels * 3 * 3)  # change the kernel to 3*3
                bias_nums.append(1)
            else:
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None, depth_map=None):
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                mask_clsses = pred_instances.labels
                # mask_score_detach = mask_scores.detach()
                mask_emb = self.classes_mask_emb(mask_clsses).unsqueeze(-1).unsqueeze(-1)
                refine_mask = (mask_scores.detach() * mask_emb).mean(dim=1, keepdims=True)
                mask_scores = (mask_scores * mask_emb.detach()).mean(dim=1, keepdims=True)
                ...
                if self.boxinst_enabled:
                    ...
                    for dilation in self.edge_dilation:
                        loss_cur_edge = comput_irn_loss(refine_mask, gt_edge, gt_bitmasks, dilation)
