        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.bg_depth:
                    ...
                    loss_feature_depth = (feature_depth_matrix * depth_norm * mask_feats.mean(1, True).sigmoid()).sum() / feature_depth_matrix.sum().clamp(
                        min=1)
                    losses.update({
                ...
                if self.boxinst_enabled:
                    ...
                    if self.depth_thresh:
                        im_inds = pred_instances.im_inds  # 实例对应图像索引
                        gt_depth = depth_prediction[im_inds].to(dtype=mask_feats.dtype)  # 每张图对应的gt_depth
                        gt_depth_bg = torch.zeros_like(gt_depth)
                        gt_depth_fg = torch.zeros_like(gt_depth)
                        gt_depth_bg[gt_depth < 30] = 1  # 确定一下阈值，虽然无法确定前景，但很远的地方必然为背景
                        gt_depth_fg[gt_depth > self.depth_thresh] = 1  # 后面再直接卡阈值或者精细做法
                        # print(gt_depth_fg.size())
                        # print(gt_bitmasks.size())
                        # print(mask_scores.size())
                        # print("*****")
                        gt_depth_fg = gt_depth_fg * gt_bitmasks  # 每个框内深度值大于阈值的那部分

                        # loss_depth_bg = torch.log((mask_scores * gt_depth_bg).sum()+1)
                        # loss_depth_fg = -torch.log((mask_scores * gt_depth_fg).sum()+1)

                        # warmup_factor = min(self._iter.item() / float(self._warmup_iters), 1.0)
                        loss_depth_bg = (mask_scores * gt_depth_bg).sum() / gt_depth_bg.sum().clamp(min=1.0)
                        loss_depth_fg = (mask_scores * gt_depth_fg).sum() / gt_depth_fg.sum().clamp(min=1.0)
                        loss_depth_bg = torch.log(loss_depth_bg + 1.0) * warmup_factor * self.hpyer_parameters[1]
                        loss_depth_fg = -torch.log(loss_depth_fg.clamp(min=0.01)) * warmup_factor * \
                                        self.hpyer_parameters[2]

                        # loss_depth_box =

                        losses.update({
                            'loss_depth_bg': loss_depth_bg,
                            'loss_depth_fg': loss_depth_fg,
                        })
                    ...
                    if self.pairwise_depth_thresh:
                        ...
                        loss_depth_pairwise = (pairwise_losses * depth_weights).sum() / depth_weights.sum().clamp(
                            min=1.0)
                        loss_depth_pairwise = loss_depth_pairwise * warmup_factor * self.hpyer_parameters[3]
                        losses.update({
                            'loss_depth_pairwise': loss_depth_pairwise,
                        })

                        if self.depth_cob_sim:
                            if not gt_depth:
                                im_inds = pred_instances.im_inds  # 实例对应图像索引
                                gt_depth = depth_prediction[im_inds].to(dtype=mask_feats.dtype)  # 每张图对应的gt_depth
                            gt_depth_fg_cob_sim = torch.ones_like(gt_depth)
                            gt_depth_fg_cob_sim[gt_depth < 30] = 0
                            gt_depth_fg_cob_sim[image_depth_similarity < self.pairwise_depth_thresh] = 0
                            loss_fg_cob_sim = (
                                                      mask_scores * gt_depth_fg_cob_sim).sum() / gt_depth_fg_cob_sim.sum().clamp(
                                min=1)
                            loss_fg_cob_sim = -torch.log(loss_fg_cob_sim.clamp(min=0.01)) * warmup_factor * \
                                              self.hpyer_parameters[2]
                            losses.update({
                                'loss_fg_cob_sim': loss_fg_cob_sim,
                            })
