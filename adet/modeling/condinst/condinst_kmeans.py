...
from .dynamic_mask_head_kmeans2 import build_dynamic_mask_head
from .mask_branch import build_mask_branch
import torchshow as ts
from adet.utils.comm import aligned_bilinear
from torchvision.utils import draw_bounding_boxes

__all__ = ["MyCondInst_Kmeans"]
...
class MyCondInst_Kmeans(nn.Module):
    """
    ...
    def add_bitmasks_from_boxes(self, instances, images, image_mask, im_h, im_w, depth_prediction=None, file_root=None):
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            if depth_prediction is not None:
                ...
                #     ], dim=0)
                img_shape = image_depth_similarity.shape[-2:]
                if self._iter >= self.kmean_v[0]:
                    ...
                    for i in range(len(per_im_gt_inst)):
                        ...
                        if box.sum() > 1000:
                            y0 = int(per_im_boxes[i][0] / 4)
                            x0 = int(per_im_boxes[i][1] / 4)
                            y1 = int(per_im_boxes[i][2] / 4)
                            x1 = int(per_im_boxes[i][3] / 4)
                            len_x = x1 - x0 + 1
                            len_y = y1 - y0 + 1
                            extend_x = int(len_x * 0.1)
                            extend_y = int(len_y * 0.1)
                            x0 = max((x0 - extend_x), 0)
                            y0 = max((y0 - extend_y), 0)
                            x1 = min((x1 + 1 + extend_x), img_shape[0])
                            y1 = min((y1 + 1 + extend_y), img_shape[1])
                            len_x = x1 - x0
                            len_y = y1 - y0
                            crop_img = depth_prediction[im_i][0, x0:x1, y0:y1]
                            img_2D = crop_img.reshape(-1, 1)
                            cluster_ids_x, cluster_centers, flag = kmeans(img_2D, num_clusters=2,
                                                                          max_itrs=self.kmean_v[1],
                                                                          device=self.device)
                            # box_mean = deepcopy(box)
                            box_mean[x0:x1, y0:y1] = (cluster_ids_x.reshape(len_x, len_y) - 0.5) * 2
                            if x0==y0==0:
                                if box_mean[x1-1, y1-1] ==1:
                                    box_mean = - box_mean
                            else:
                                if box_mean[x0, y0] == 1:  # 保证bg部分的标签是-1,前景1
                                    box_mean = - box_mean
                            if flag:
                                box_mean[:] = 0
