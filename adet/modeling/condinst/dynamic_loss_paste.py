...
def comput_irn_loss(mask_scores, gt_edge, gt_bitmasks, dilation):
    ...
    diff[diff == 0] = 0
    diff[diff > 5] = 0
    is_differnet = 1 #(mask_scores>0.4) * ( mask_scores<0.6)
        ...
        if self.training and branch == 'student':
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    image_color_similarity = image_color_similarity[gt_inds_all].to(dtype=mask_feats.dtype)

                    loss_prj_term = compute_project_term(mask_scores, gt_bitmasks)

                    pairwise_losses = compute_pairwise_term(
                        mask_logits_all, self.pairwise_size,
                        self.pairwise_dilation
                    )

                    weights = (image_color_similarity >= self.pairwise_color_thresh).float() * gt_bitmasks_all.float()
                    ...
                    if self.hpyer_parameters[4]:
                        loss_edge = 0 #*mask_scores.sum()
                        if 1: #gt_instances[-1].has('pseudo_masks'):
                    ...
                    if (self._iter > self._warmup_iters) and self.pairwise_depth_thresh:
                        ...
                        loss_depth_pairwise = (pairwise_losses[im_inds_all<2] * depth_weights).sum() / depth_weights.sum().clamp(
                            min=1.0)
