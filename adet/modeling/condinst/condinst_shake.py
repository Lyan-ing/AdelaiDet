...
from .dynamic_mask_head_irn_DP import build_dynamic_mask_head
...
class MyCondInst_Shake(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        self.shake_factors = [0.2,0.15,0.1,0.05,0.02, 0]
        self.shake_factor = 0
    ...
    def forward(self, batched_inputs):
        self._iteration +=1
        self.shake_factor = self.shake_factors[min(int(self._iteration / 7330), 5)]
        # original_images = [x["image"].to(self.device) for x in batched_inputs]
        if self.use_depth and self.training:
        ...
        if self.training:
            mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances, depth_map=depth_prediction)
    ...
    def add_bitmasks_from_boxes(self, instances, images, image_masks, im_h, im_w, depth_prediction=None):
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            for per_box in per_im_boxes:
                ...
                if self.shake_factor:
                    box_h = int((per_box[3] - per_box[1]) * self.shake_factor) + 1
                    box_w = int((per_box[2] - per_box[0]) * self.shake_factor) + 1
                    shake_t = random.choice(range(box_h))
                    shake_b = random.choice(range(box_h))
                    shake_l = random.choice(range(box_w))
                    shake_r = random.choice(range(box_w))
                    # bitmask_full[int(per_box[1] - shake_t):int(per_box[3] + 1 + shake_b), int(per_box[0] - shake_l):int(per_box[2] + 1 + shake_r)] = 1.0
                    bitmask_full[max(int(per_box[1]) - shake_t, 0):min(int(per_box[3] + 1) + shake_b, im_h),
                                max(int(per_box[0]) - shake_l, 0):min(int(per_box[2] + 1 + shake_r), im_w)] = 1.0
                else:
