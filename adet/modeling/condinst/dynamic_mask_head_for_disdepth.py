...
def comput_irn_loss(mask_scores, gt_edge, gt_bitmasks, dilation):
    ...
    # diff = (torch.stack([left_top, top, right_top, left], 1)).abs().max(1, True)[0]
    ...
    if len(mask_edge) == 0:
        ...
        loss_edge = - torch.log((mask_edge ** 2).mean() + 1e-5)
    return loss_edge
...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.register_buffer("_iterss", torch.zeros([1]))
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None, depth_map=None,
        ...
        if self.training and branch == 'student':
            ...
            gt_inds_all = pred_instances.gt_inds  # 预测实例对应真实实例的索引（一个batch所有图像的所有实例数）
            im_inds_all = pred_instances.im_inds  # 实例对应图像索引
            im_inds = im_inds_all[im_inds_all<2]
            gt_inds = gt_inds_all[im_inds_all<2]
            gt_bitmasks_all = torch.cat([per_im.gt_bitmasks for per_im in gt_instances])
            gt_bitmasks_all = (gt_bitmasks_all[gt_inds_all]).unsqueeze(dim=1).to(dtype=mask_feats.dtype)
            box_gt_bitmasks = deepcopy(gt_bitmasks_all)
            gt_bitmasks = gt_bitmasks_all[im_inds_all<2]
            gt_edge = torch.cat([per_im.depth_edge for per_im in gt_instances[:2]])
            ...
            if len(pred_instances) == 0:
                ...
                mask_logits_all, depth_logits = self.mask_heads_forward_with_coords(
                    mask_feats, mask_feat_stride, pred_instances
                )
                mask_scores_all = mask_logits_all.sigmoid()
                mask_logits = mask_logits_all[im_inds_all<2]
                mask_scores = mask_scores_all[im_inds_all<2]
                depth_logits = depth_logits[im_inds_all<2]
                # mask_logits_new = mask_logits_all[im_inds>=2]
                # mask_scores_new = mask_scores_all[im_inds>=2]
                ...
                if self.boxinst_enabled:
                    warmup_factor = min(self._iterss.item() / float(self._warmup_iters), 1.0)
                    # depth_pred

                    if gt_instances[-1].has('pseudo_masks'):
                        pseudo_masks = torch.cat([per_im.pseudo_masks for per_im in gt_instances])
                        pseudo_masks = pseudo_masks[im_inds_all].unsqueeze(dim=1).to(dtype=mask_feats.dtype)
                        pseudo_masks = pseudo_masks * gt_bitmasks_all
                        pseudo_scores = torch.cat([per_im.pseudo_scores for per_im in gt_instances])
                        mask_flags = pseudo_scores = pseudo_scores[im_inds_all].to(dtype=mask_feats.dtype)
                        # if self.tmp:
                        # pseudo_masks = (mask_scores.detach() > 0.5).float() * pseudo_masks
                        loss_pseudo_masks = dice_coefficient(mask_scores_all, pseudo_masks)
                        ...
                        losses.update({
                            'loss_pseudo_masks': mask_scores_all.mean() * 0.,
                        })
                        # losses.update({
                        #     'loss_pseudo_depths': depth_logits.mean() * 0.,
                        # })
                    ...
                    loss_prj_term = compute_project_term(mask_scores_all, gt_bitmasks_all)
                    ...
                    if (self._iterss > self._warmup_iters) and self.pairwise_depth_thresh:
                        # import torchshow as rs
                        image_depth_similarity = torch.cat([x.image_depth_similarity for x in gt_instances])
                        # rs.save(image_depth_similarity[0], '/home/yl/python/AdelaiDet1/out/sim.png')
                        image_depth_similarity = image_depth_similarity[gt_inds].to(dtype=mask_feats.dtype)
                        # image_depth_similarity = image_depth_similarity* gt_bitmasks.float()
                        depth_weights = (image_depth_similarity >= self.pairwise_depth_thresh).float()
                        # depth_weights = depth_weights * gt_bitmasks.float()
                        # rs.save(depth_weights[3][0], '/home/yl/python/AdelaiDet1/out/dsim8.png')
                        # depth_weights = (image_depth_similarity >= 0.3).float()
                        # rs.save(depth_weights[0][0], '/home/yl/python/AdelaiDet1/out/dsim3.png')
                        # depth_weights = (image_depth_similarity >= 0.5).float()
                        # rs.save(depth_weights[0][0], '/home/yl/python/AdelaiDet1/out/dsim5.png')
                        if self.depth_with_box:
                            depth_weights = depth_weights * gt_bitmasks.float()
                        loss_depth_pairwise = (pairwise_losses * depth_weights).sum() / depth_weights.sum().clamp(
                            min=1.0)
                        warmup_factor2 = min((self._iterss.item() - self._warmup_iters) / float(self._warmup_iters), 1.0)
                        ...
                        #     # else:
                        #     mask_affinity_weights = (mask_affinity >= 0.3).float() * box_gt_bitmasks.float()

                        #     loss_mask_affinity = (pairwise_losses * mask_affinity_weights).sum(
                        #         [1, 2, 3]) / mask_affinity_weights.sum([1, 2, 3]).clamp(min=1.0)
                        #     # NOTE: weight???
                        #     loss_mask_affinity = (loss_mask_affinity[
                        #         mask_flags > 0]).mean()  # (loss_mask_affinity * pseudo_mask_weight).sum() / num_valid_masks
                        ...
                        #     # if self.with_warmup:
                        #     #     warmup_factor = self.mask_warmup_fn(self._iterss)
                        #     # else:
                        #     #     warmup_factor = 1.0
                        #     # losses["loss_mask"] = loss_mask * self.mask_weight * warmup_factor
                        #     losses[
                        #         "loss_mask_affinity"] = loss_mask_affinity * 1 * 0.5  # self.pseudo_mask_affinity_weight
