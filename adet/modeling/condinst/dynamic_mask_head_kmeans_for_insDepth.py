...
class DynamicMaskHead(nn.Module):
    ...
    def mask_heads_forward_with_coords(
        ...
        mask_logits, depth_logits = self.mask_heads_forward(mask_head_inputs, weights, biases, n_inst)  # 获得controller的参数
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    if (self._iter > self.kmean_v[0]):
                        ...
                        mask_bg = mask_scores * [filter_no_edge_bitmask == -1][0]
                        loss_kmean = (mask_bg.sum(dim=(1,2,3)) / [filter_no_edge_bitmask == -1][0].sum(dim=(1,2,3)).clamp(min=1.0))
                        if len(loss_kmean[loss_kmean>0]) ==0:
                            loss_kmean = mask_bg.sum() *0.
                            # print(loss_kmean[loss_kmean > 0])
                            # print('++++++++++++++')
                        else:
                            # print(len([loss_kmean>0]))
                            #
                            # print(loss_kmean[loss_kmean>0])
                            # print('---------------')
                            loss_kmean = loss_kmean[loss_kmean>0].mean()
                            # print(loss_kmean)

                        # filter_no_edge_bitmask[depth_kmean!= 1]=1  # 背景
                        # filter_no_edge_bitmask[depth_kmean == 1] = 0  # 前景
                        # loss_kmean = dice_coefficient((1-mask_scores), filter_no_edge_bitmask).mean()
                        ...
                        # filter_no_edge_bitmask[end_bitmask_sum_h > (ori_bitmask_sum_h - 4)] = 0
                        #
                        ...
                        # filter_no_edge_bitmask[end_bitmask_sum_w > (ori_bitmask_sum_w - 4)] = 0
                        #
                        ...
                        losses.update({
                            'loss_kmean': loss_kmean * self.kmean_v[2],
                        })
