...
# from .dynamic_mask_head_irn_DP_for_CrossDepth import build_dynamic_mask_head
...
def get_images_color_similarity(images, image_masks, kernel_size, dilation, sim=0.1):
    ...
    if images.size(1) == 1:
        ...
        edge = (edge < sim)  # ts.save(images[0], '/media/lyan/database/python/ttmp/images.png')
...
class MyCondInst_IRN_Cins(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.BOXINST.CROSS == 'aug':
            from .dynamic_mask_head_irn_DP_for_CrossDepth_aug import build_dynamic_mask_head
        else:
            from .dynamic_mask_head_irn_DP_for_CrossDepth import build_dynamic_mask_head
